...
```markdown
[Builded] Version 2.7.7 - 77 / 2.7.7 - 77 _ feat: Update the UI of the game screen
...
Flutter build for Android
```flutter build appbundle --build-name=2.7.7 --build-number=77 --release```
...
#### Command for copilot
    ...
    Generate the message using the `Git message template`, `Store notices, what is news? sumarry and more general message, in vi and en, brief it short and summary, don't mention any things could make the concern by information security converning, which could be risk in Apple Store, and Play Store` , `Flutter build for Android`, `Flutter build for iOS`
```markdown
[Builded] Version 2.7.10 - 80 / 2.7.10 - 80 _ feat: Upgraded the UI of the game screen

**Store notices / What's new / Summary:**

- **English:**  
    - Upgraded the game screen UI with an improved quick settings menu for multiplayer mode and refined visual effects for level-ups and extra lives, ensuring a more polished and engaging experience.

- **Tiếng Việt:**  
    - Nâng cấp giao diện màn hình trò chơi với menu cài đặt nhanh cho chế độ nhiều người chơi và tinh chỉnh các hiệu ứng hình ảnh khi lên cấp và nhận thêm mạng, mang lại trải nghiệm mượt mà và hấp dẫn hơn.

Flutter build for Android
```flutter build appbundle --build-name=2.7.10 --build-number=80 --release```
...
```markdown
[Builded] Version 2.8.3 - 84 / 2.8.2 - 83 _ feat: Migrate to push notifications and optimize greeting messages

**Store notices / What's new / Summary:**

- **English:**  
    - Improved notification delivery for a more reliable experience.
    - Optimized greeting messages with daily caching for faster load times.

- **Tiếng Việt:**  
    - Cải thiện hệ thống thông báo để trải nghiệm ổn định hơn.
    - Tối ưu lời chào với bộ nhớ đệm hàng ngày, tải nhanh hơn.

Flutter build for Android
```flutter build appbundle --build-name=2.8.3 --build-number=84 --release```
...
`[Builded] Version 2.8.5 - 86 / 2.8.4 - 85 _ feat: Optimize UI UX`

#### Brief for Store's new update:

_ Improved app interface and user experience with refined visual design and smoother navigation.
_ Cải thiện giao diện và trải nghiệm người dùng với thiết kế trực quan hơn và điều hướng mượt mà hơn.

#### Flutter build for Android for Testing (Development)

`npm run flutterfire_setup_dev && flutter build appbundle --build-name=2.8.5.0 --build-number=86 --release --dart-define=APP_ENVIRONMENT=DEVELOPMENT`

#### Flutter build for iOS for Testing (Development)

`npm run flutterfire_setup_dev && flutter build ios --build-name=2.8.4 --build-number=85.0 --release --dart-define=APP_ENVIRONMENT=DEVELOPMENT`

#### Flutter build for Android

`npm run flutterfire_setup && flutter build appbundle --build-name=2.8.5 --build-number=86 --release --dart-define=APP_ENVIRONMENT=PRODUCTION`
